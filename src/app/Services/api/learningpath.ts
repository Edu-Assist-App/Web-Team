...
// 6. Update progress for a specific step in a learning path (POST)
export async function updateStepProgress(
  pathId: string,
  stepId: string,
  params: UpdateStepProgressParams
): Promise<UserProgress> {
  const { data } = await axiosInstance.post<UserProgress>(
    `/api/v1/learning-paths/progress/${pathId}/step/${stepId}`,
    null, // POST with query params, no request body
    { params }
  );
  return data;
}

// 7. Get all steps for a learning path by course ID (GET)
export async function getLearningPathStepsByCourseId(
  courseId: string
): Promise<LearningPathStep[]> {
  const { data } = await axiosInstance.get<LearningPathStep[]>( // Assuming LearningPathStep is the correct type for individual steps
    `/api/v1/learning-paths/course/${courseId}/steps`
  );
  return data;
}