import { Button } from "@/app/components/ui/button";
import { Card, CardContent } from "@/app/components/ui/card";

interface StudyMaterialCardProps {
  id: number;
  title: string;
  description: string;
  image: string;
}

export function StudyMaterialCard({
  id,
  title,
  description,
  image,
}: StudyMaterialCardProps) {
  return (
    <Card className="flex flex-col items-start gap-[13.24px] p-1 rounded-[10.39px]">
      <div className="relative w-full">
        <div
          className="w-full h-[194.64px] rounded-lg bg-cover bg-center"
          style={{ backgroundImage: `url(${image})` }}
        />
        <Button
          variant="outline"
          size="icon"
          className="p-1.5 absolute top-2 right-2 bg-[#f9f9f9] rounded-[28px] border border-solid border-[#00000003]"
        >
          <img className="w-6 h-6" alt="Options" src="/frame-7.svg" />
        </Button>
      </div>

      <CardContent className="flex flex-col items-start gap-[9.93px] p-0 w-full">
        <div className="flex flex-col items-start justify-center gap-[5.56px] w-full">
          <h4 className="font-['Poppins',Helvetica] font-medium text-[#000000b2] text-base w-full">
            {title}
          </h4>
          <p className="font-['Poppins',Helvetica] font-normal text-[#000000b2] text-[10px] w-full">
            {description}
          </p>
        </div>
      </CardContent>

      <Button className="px-6 py-2 bg-[#3800b3] rounded-[99px] text-white font-['Poppins',Helvetica] font-medium text-xs">
        View
      </Button>
    </Card>
  );
}
